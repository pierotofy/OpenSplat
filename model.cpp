...
int countPrefixes(const char *prefix, std::ifstream &f, std::string &line){
    int n = 0;
    while(true){
...
int Model::loadPly(const std::string &filename){
    std::error_code ec;
    const std::uintmax_t fileSize = fs::file_size(filename, ec); // NOTE: Throws error if file doesn't exist
    if (ec){
        throw std::runtime_error(std::string("Invalid PLY snapshot file: ") + ec.message());
    }
    ...
    if (line == "ply"){
        ...
        if (line == "format binary_little_endian 1.0"){
            std::getline(f, line);
            numBytesRead += f.gcount();
            if (line.rfind("comment Generated by opensplat at iteration ", 0) == 0){
                step = std::stoi(line.substr(44)); // NOTE: Ignores trailing non-numeric numbers, e.g. stoi("567x") returns 567 instead of raising an error
                if (step != 0){
                    std::getline(f, line);
                    numBytesRead += f.gcount();
                    if (line.rfind("element vertex ", 0) == 0){
                        const int numPoints = std::stoi(line.substr(15));
                        
                        const char *requiredProperties1[6] = {
                            "property float x",
                            "property float y",
                            "property float z",
                            "property float nx",
                        ...
                        for (int i = 0; i < 6; i++){
                            std::getline(f, line);
                            numBytesRead += f.gcount();
                            if (line != requiredProperties1[i]){
                                throw std::runtime_error(std::string("PLY snapshot file's header does not contain required property: ") + requiredProperties1[i]);
                            }
                        }
                        std::getline(f, line);
                        numBytesRead += f.gcount();
                        int featuresDc_size1 = countPrefixes("property float f_dc_", f, line);
                        int featuresRest_size1 = countPrefixes("property float f_rest_", f, line);
                        
                        const char *requiredProperties2[9] = {
                        ...
                        for (int i = 0; ; i++){
                            if (line != requiredProperties2[i]){
                                throw std::runtime_error(std::string("PLY snapshot file's header does not contain required property: ") + requiredProperties2[i]);
                            }
                            if (i == 8)
                                break; // This is to avoid calling std::getline after "end_header" has been received, because all data afterwards is binary
                            std::getline(f, line);
                            numBytesRead += f.gcount();
                        }
                        
                        // NOTE/TODO: I'm assuming tensor.cpu() is just a way to copy its contents to CPU RAM instead of GPU VRAM, (thus featuresDcCpu.size(1) === featuresDc.size(1))... I'll have a look to verify this
                        const size_t bytesPerPoint = sizeof(float) * 3 + sizeof(float) * 3 + sizeof(float) * featuresDc_size1 + sizeof(float) * featuresRest_size1 + sizeof(float) * 1 + sizeof(float) * 3 + sizeof(float) * 4;
                        const size_t remainingFileSize = fileSize - numBytesRead;
                        if (remainingFileSize != bytesPerPoint * numPoints){
                            std::cout << "Loading snapshot into model..." << std::endl;
                            
                            float zeros[3];

                            torch::Tensor featuresRestCpu = torch::zeros({numPoints, featuresRest_size1}, torch::TensorOptions().dtype(torch::kFloat32));
                            torch::Tensor meansCpu = torch::zeros({numPoints, 3}, torch::TensorOptions().dtype(torch::kFloat32));
                            torch::Tensor featuresDcCpu = torch::zeros({numPoints, featuresDc_size1}, torch::TensorOptions().dtype(torch::kFloat32));
                            ...
                            for (size_t i = 0; i < numPoints; i++){
                                ...
                                f.read(reinterpret_cast<char *>(featuresDcCpu[i].data_ptr()), sizeof(float) * featuresDc_size1);
                                f.read(reinterpret_cast<char *>(featuresRestCpu[i].data_ptr()), sizeof(float) * featuresRest_size1);
                            ...
                            // Copy from CPU to GPU (if using GPU)
                            featuresRest = featuresRestCpu.reshape({numPoints, 3, featuresRest_size1/3}).transpose(2, 1).detach().to(device).requires_grad_();
                            means = meansCpu.detach().to(device).requires_grad_();
                            featuresDc = featuresDcCpu.detach().to(device).requires_grad_();
                            opacities = opacitiesCpu.detach().to(device).requires_grad_();
                            scales = scalesCpu.detach().to(device).requires_grad_();
                            quats = quatsCpu.detach().to(device).requires_grad_();
                            
                            std::cout << "Loaded " << means.size(0) << " gaussians" << std::endl;
                            ...
                            throw std::runtime_error("PLY snapshot file's data section is wrong size");
                        }
                    }
                } else {
                    throw std::runtime_error("PLY snapshot file failed sanity check: iteration count should not begin at 0");
                }
            } else if (line.rfind("comment Generated by opensplat")){
                throw std::runtime_error("PLY snapshot file does not contain iteration count metadata. You can edit the file to add this metadata manually, by changing \"comment Generated by opensplat\" to \"comment Generated by opensplat at iteration 12345\", changing 12345 to the desired value.");
            }
        }
    }
    throw std::runtime_error("Invalid PLY snapshot file");
}
