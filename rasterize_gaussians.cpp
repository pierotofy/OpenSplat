...
variable_list RasterizeGaussians::forward(AutogradContext *ctx, 
    ...
    torch::Tensor finalAcc = std::get<2>(t);

    torch::Tensor finalTs = std::get<3>(t);

    // Map of tile bin IDs
    torch::Tensor finalIdx = std::get<4>(t);

    ctx->saved_data["imgWidth"] = imgWidth;
    ctx->saved_data["imgHeight"] = imgHeight;
    ctx->save_for_backward({ gaussianIdsSorted, tileBins, xys, depths, conics, colors, opacity, background, finalTs, finalIdx, finalAcc, outDepth });
...
tensor_list RasterizeGaussians::backward(AutogradContext *ctx, tensor_list grad_outputs) {
    ...
    torch::Tensor finalAcc = saved[10];
    torch::Tensor finalDepth = saved[11];
    ...
    torch::Tensor v_depths = std::get<4>(t);
